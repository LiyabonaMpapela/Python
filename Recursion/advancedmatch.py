# determine if a given pattern matches a given 
#Liyabona Mpapela
#6 October 2022
...
def match(pattern, word):
    ...
    if pattern==word:#checks if the pattern and the word are the same 
        return True
    if pattern=='*':#checks if the pattern could be anything
        return True
    ...
    if pattern[0]=='*' :#checks if the pattern has a star on a position
        return match(pattern[1:],word) or match(pattern,word[1:])# checks if the letters of the pattern and the word match on the positions
    return False
